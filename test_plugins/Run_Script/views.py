...
def rs_run(request):
    ...
    a = api()
    print("DEBUG : run script")
    print(request.GET.get('form'))
    fil = request.GET.get('form')['script_file']
    fp = open('/srv/salt/tmp_script.sh', 'w')
    fp.write(fil.read())
    jid = a.run_command(request.POST.getlist('minions[]'), 'cmd.script', args=['salt://tmp_script.sh'], expr_form='list', task='Run Script')
