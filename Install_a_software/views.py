...
def install_input(request):
    print("DEBUG : Inside install_input..")
    template = Template(open('Install_a_software/install_input.html').read())
...
def install_run(request):
    ...
    jid = a.run_command(request.POST.getlist('minions[]'), 'pkg.install', args=[request.POST.get('soft')], expr_form='list', task='install')
